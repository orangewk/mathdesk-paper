...
![Concept graph for the correlation coefficient domain](figures/fig2-concept-graph.pdf)
...
*Ethical considerations for all three experiments are discussed in Section 6.3. Experimental tutor responses are entirely LLM-generated; no human learner utterances appear in the reported data. Profile and probe design for the factoring experiment were constructed in interactive sessions with Anthropic Claude, informed by the author's prior tutoring observations; see Section 6.2 (L3+L4) and Section 6.3.*
...
**Research ethics.** No human learner utterances or identifiers appear in the data analyzed in Sections 4–5; experimental tutor responses are entirely LLM-generated. Profile design for the Galaxy factoring experiment was conducted in interactive sessions with Anthropic Claude, informed by the author's prior informal tutoring observations of secondary-school learners (a household setting; no formal IRB process; no specific individual's mastery state encoded; no learner transcripts in the reported data). Replication studies should construct profiles from independent learner data sources or expert pedagogical judgment.

**Rights of human learners using a deployed system: principled withholding and opt-out.** When this architecture is deployed as a learner-facing tutoring system used by a human learner, the LLM serving as the tutor is configured by the principle layer (Layer 1, embedded in the system prompt) to delay the formal name of a concept and the canonical procedure until the human learner reaches the point of needing them — the design choice grounded in Guided Reinvention (§3.2). This delay is pedagogically defensible while the human learner is actively making progress through the discovery sequence. It becomes harmful when the human learner is left in unresolved confusion: at that point, the LLM tutor's withholding the answer is no longer pedagogy but neglect of the human user.

A responsible deployment must therefore specify two safeguards governing the LLM tutor's behavior toward the human learner. First, an explicit *abandonment trigger*: the LLM tutor must exit discovery mode and provide the formal explanation to the human learner when, for example, several successive attempts to descend have failed, or whenever the human learner explicitly asks for the answer. Second, an *opt-out*: the human learner must be able to choose, at any time, to receive a direct explanation from the LLM tutor instead of the discovery-first sequence.
...
## 6.5 Implications and Misreadings to Avoid

The architectural validation reported here reframes the problem of LLM mathematics tutoring. The dominant question in the field has been whether models possess sufficient mathematical competence to teach — a knowledge question. The results above suggest the question to ask next is different: given that competence is sufficient, *how is the deployment of that competence governed for a specific learner*. That is a control question, not a knowledge question, and the four-layer architecture is a proposal for what such control infrastructure looks like. KST contributes the formal vocabulary in which the control target — the learner's knowledge state — can be specified; the strategy few-shot and the externalized concept map together translate that specification into operational tutoring behavior.
