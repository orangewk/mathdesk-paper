...
When a learner does not understand a concept, the critical question is not "what else can I say about this concept?" but "what does this specific learner already know, and what prerequisite layer is missing?" These are structurally different questions. The first operates on the concept; the second operates on the learner\textquotesingle s knowledge state relative to the concept\textquotesingle s prerequisite graph. Answering the second question requires three things: a representation of the learner\textquotesingle s acquired concepts, a map of the concept\textquotesingle s prerequisite structure, and a principle that governs how the tutor should move through that structure in response to comprehension signals.
...
A precise formulation of the gap: \emph{no existing system governs an LLM\textquotesingle s teaching route through a concept\textquotesingle s prerequisite graph based on a formal representation of the learner\textquotesingle s acquired knowledge state.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Four-layer control architecture}]{figures/fig1-architecture.pdf}}

\textbf{Figure 1.} Four-layer control architecture. Layers 1--3 embed in the system prompt as standing constraints. Layer 4 is invoked as an external tool at each conversational turn.
...
Together, the four principles express a single operation --- Building-Blocks decomposition --- in which an unknown concept is recursively reduced to components in the learner\textquotesingle s acquired knowledge. The principle layer occupies \textasciitilde40 of the total \textasciitilde80 prompt lines, encoding the constraint through direct statement and the strategy few-shot (Section 3.4).
...
The decision to represent the learner profile \emph{exclusively} as an acquired-concept set was validated by the H1 ablation study (Section 4.2), which varied three components across 475 runs (400 Gemini, 75 Claude):
...
\item
  Connect that knowledge to the target concept via the prerequisite structure encoded in the model\textquotesingle s internal representation.
\item
  Engage the known knowledge first (experience), let the learner observe the emerging pattern (discovery), and name the result last (formalization).
\end{enumerate}
...
\textbf{The \texttt{where\_are\_we} tool.} The tool accepts a natural-language observation from the tutor and returns a structured navigation report classifying each concept node as \emph{confident}, \emph{fuzzy}, or \emph{unknown}, then computing:
...
A \emph{surveyor} sub-agent converts the tutor\textquotesingle s observation into node-level status estimates; a deterministic graph step produces frontier, obstacle, and detour outputs. The full tool schema, surveyor configuration, and graph algorithms are documented in Appendix D.

\textbf{Embedding vs. tool.} In Galaxy v0, the map was embedded as JSON in the system prompt. All five runs ignored it and followed the same default route. In v1, provided as \texttt{where\_are\_we}, the tutor called the tool at each turn and produced profile-dependent paths across all three profiles (15/15). Replication on factorization confirmed generalization (9/9 profile-dependent routes, 3/3 obstacle detections). The mechanism reflects a general property: embedded text may be bypassed; tool invocation creates an explicit retrieval event the model incorporates.
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Concept graph for the correlation coefficient domain}]{figures/fig2-concept-graph.pdf}}

\textbf{Figure 2.} Concept graph for the correlation coefficient domain. Core nodes (confidence 1.0) in dark green; medium-confidence nodes (0.9) in light green; mid-band node (0.5) in amber; fringe node (0.4) in pale yellow. Core edges (confidence $\geq$ 0.7) shown as solid lines; fringe edges as dashed lines.
...
\subsection{References (Section 3)}\label{references-section-3}
...
\subsection{4.0 Research Program Scope}\label{40-research-program-scope}
...
\textbf{Domain transfer.} The algebra few-shot applied without modification to P-stat and P-geom produced discovery-first behavior in both domains (Section 5.1).
...
\emph{Ethical considerations for all three experiments are discussed in Section 6.3. No human learners participated in data collection; all learner profiles and probes were researcher-constructed.}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{References (Section 4)}\label{references-section-4}
...
\textbf{Figure 3.} K-effect replication (P-stat). K-containing conditions: 5/5 covariance references; all non-K conditions: 0/5. Pattern held across Gemini and Claude.
...
Externalizing the concept map as an MCP tool (\texttt{where\_are\_we}) causes profile-dependent route selection; embedding the same map in the system prompt produces no graph-guided behavior. Across 30 v1 runs (24 Turn-1 + 6 Turn-2), every run was consistent with the graph\textquotesingle s frontier computation and the learner\textquotesingle s acquired concept set.
...
\textbf{Table 12.} v1 correlation coefficient: profile × route × consistency. All three routes differ from each other and from the v0 baseline.
...
\textbf{Table 13.} v1 factoring: profile × teaching route × consistency. Turn 2 (3 runs, A-mid): the tutor guided the learner to notice \(3+5=8\), \(3 \times 5=15\) within an already-computed expansion, constructing the factoring discovery from a confirmed skill.

\subsubsection{5.3.4 Full Score Summary}\label{534-full-score-summary}

\pandocbounded{\includegraphics[keepaspectratio,alt={v0 vs. v1 teaching routes (correlation coefficient)}]{figures/fig4-galaxy-routes.pdf}}

\textbf{Figure 4.} v0 vs. v1 teaching routes (correlation coefficient). v0: all five runs converged on scatter plot; Turn 2 regressed to mean. v1: routes diverged by profile; obstacle detected and re-routed to confident ancestor.
...
\subsection{References (Section 5)}\label{references-section-5}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Four-layer architecture mapped to KST-interpretable roles}]{figures/fig5-kst-mapping.pdf}}
...
\textbf{L3 + L4: Methodological scope.} In all experiments, the learner\textquotesingle s knowledge state \emph{K} was initialized manually using researcher-constructed profiles. The lateral-movement problem framing was partly motivated by retrospective observations from pilot tutoring sessions conducted by the authors, introducing a circularity risk between problem identification and architectural design. Independent evidence from GuideEval (Liu et al., 2025), finding low orchestration-strategy adaptivity across multiple LLM tutoring systems, substantially reduces this concern for the problem framing; however, automatic \emph{K} estimation from conversation remains unimplemented. A deployed system requires either a pre-session assessment or an online estimation mechanism.
...
\subsection{References (Section 6)}\label{references-section-6}
...
The author thanks his daughter, whose mathematics learning sessions originally motivated this work. Among the large language models used in this research, Claude (Anthropic) was the primary tool for drafting, review, and concept-graph construction; Gemini (Google), GPT-5.4 / Codex (OpenAI), and Qwen (Alibaba) contributed in cross-model audits. All design decisions, experimental analyses, and final claims are the author\textquotesingle s own.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section*{Data and Code Availability}\label{data-and-code-availability}

The strategy few-shot prompt (Appendix C), the \texttt{where\_are\_we} tool schema and surveyor configuration (Appendix D), the HIL labeling rubric (Appendix E), and the integrated reference list are included with this paper. The concept graph used in Experiments 1 and 3, the experiment scripts, and the raw conversation logs are maintained in a project repository; the public location and persistent identifier are listed at the URL provided in the camera-ready version. For the review version, materials beyond those reproduced in the appendices are available from the corresponding author on request, subject to anonymity preservation. All artifacts will be deposited under a permissive license at the time of publication.
...
{[}2T-KT{]} (2025). Thinking As a Teacher: LLM Supporting Knowledge Tracing Leveraging Global Subject and Student Specific Knowledge Graphs. \emph{Information Fusion} (Elsevier). ScienceDirect PII: S1566253525006499. {[}TBD: full author list and DOI --- paper title and journal venue are confirmed via the public implementation repository (github.com/linqingliDestinyController/2T-KT-model) and the ScienceDirect listing (\url{https://www.sciencedirect.com/science/article/abs/pii/S1566253525006499}), but the full author list and DOI could not be retrieved without journal access{]}.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Extraction notes}\label{extraction-notes}

\begin{itemize}
\tightlist
\item
  \textbf{Total citations extracted from body text:} 21 distinct citation keys
\item
  \textbf{Bibliographic details confirmed from existing section References:} 8
  ...
  \end{itemize}
\item
  \textbf{Verified via web search on 2026-04-24/25:} 11
  ...
  \item
    Jiang et al. (2025) --- MAS-KCL, \emph{The Visual Computer} / CGI 2025 (arXiv:2505.14126)
  \item
  ...
  \item
    Yang et al. (2025) --- \emph{Machine Learning and Knowledge Extraction} 7(3):103 (MDPI), replacing the unverifiable Giabbanelli (2025) citation
  \end{itemize}
\item
  \textbf{TBD remaining (could not verify):} 1

  \begin{itemize}
  \tightlist
  \item
    2T-KT (2025) --- title and journal (\emph{Information Fusion}) confirmed via GitHub implementation repo and ScienceDirect listing (PII S1566253525006499); full author list could not be retrieved
  \end{itemize}
\end{itemize}

\subsubsection{Disambiguation note}\label{disambiguation-note}
...
\tightlist
\item
  \textbf{Liu et al. (2024)} = SocraticLM (Section 2.2, Socratic Tutoring) --- first author \textbf{Jiayu Liu} (USTC group, Huang/Chen et al.)
\item
  \textbf{Liu et al. (2025)} = GuideEval (Sections 1, 2.2, 3.2, 6.2, 7) --- first author \textbf{Ying Liu} (different research group; Hua Huang et al.)
\end{itemize}
...
\section{Appendix C. Strategy Few-Shot Layer --- Actual Prompt Text}\label{appendix-c-strategy-few-shot-layer--actual-prompt-text}
...
\subsection{C.1 Relationship to the Paper}\label{c1-relationship-to-the-paper}
...
\subsection{C.2 System Prompt Structure}\label{c2-system-prompt-structure}
...
\subsection{C.3 Character and Output Constraint Block}\label{c3-character-and-output-constraint-block}
...
\subsection{C.4 Principle Layer (Layer 1)}\label{c4-principle-layer-layer-1}
...
\subsection{C.5 Output Constraint Block (Continued in Principle Layer)}\label{c5-output-constraint-block-continued-in-principle-layer}
...
\subsection{C.6 Concept Map Navigation Block (Layer 4 Integration)}\label{c6-concept-map-navigation-block-layer-4-integration}
...
\subsection{C.7 Anonymization Notes}\label{c7-anonymization-notes}
...
\subsection{C.8 Line Count}\label{c8-line-count}
...
\bottomrule\noalign{}
\endlastfoot
Character / output constraints (§C.3) & \textasciitilde25 \\
Principle layer / learning sequence (§C.4) & \textasciitilde20 \\
Output constraints continued (§C.5) & \textasciitilde25 \\
Concept map navigation (§C.6, C2 variant) & \textasciitilde20 \\
\textbf{Total} & \textbf{\textasciitilde90} \\
\end{longtable}
}

The count slightly exceeds the "\textasciitilde80 lines" cited in §3.2 because §C.5 includes visual-expression constraints not counted in the §3.2 estimate. The pedagogically load-bearing text (§C.4, principle + learning sequence) occupies approximately 20 lines; the remainder is formatting and tool-call protocol.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{\texorpdfstring{Appendix D. Surveyor (\texttt{where\_are\_we}) --- Technical Specification}{Appendix D. Surveyor (where\_are\_we) --- Technical Specification}}\label{appendix-d-surveyor-where_are_we--technical-specification}
...
\subsection{D.1 Overview}\label{d1-overview}
...
\subsection{D.2 Tool Interface}\label{d2-tool-interface}
...
\subsection{D.3 Concept Node Representation}\label{d3-concept-node-representation}
...
\subsection{D.4 Surveyor Sub-Agent}\label{d4-surveyor-sub-agent}
...
\subsection{D.5 Knowledge State Record (Persistent Storage)}\label{d5-knowledge-state-record-persistent-storage}
...
\subsection{D.6 computeTerrain Algorithm}\label{d6-computeterrain-algorithm}
...
\subsection{D.7 Additional Tools}\label{d7-additional-tools}
...
\subsection{D.8 Anonymization Notes}\label{d8-anonymization-notes}
...
\section{Appendix E. HIL Rubric --- Tutor Response Classification}\label{appendix-e-hil-rubric--tutor-response-classification}
...
\subsection{E.1 Two Classification Instruments}\label{e1-two-classification-instruments}
...
\subsection{E.2 Five-Point HIL Quality Rubric (P1--P5)}\label{e2-five-point-hil-quality-rubric-p1p5}
...
\subsection{E.3 Three-Category Response Classifier}\label{e3-three-category-response-classifier}
...
\subsection{E.4 Inter-Rater Agreement}\label{e4-inter-rater-agreement}
...
\subsection{E.5 Sample Size and Labeling Protocol}\label{e5-sample-size-and-labeling-protocol}
...
Inter-rater agreement & $\kappa$ = 0.831 on 20-response sample (see §E.4) \\
\end{longtable}
}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{E.6 TBD Items}\label{e6-tbd-items}
