...
\pandocbounded{\includegraphics[keepaspectratio,alt={Lateral movement vs. KST-guided routing}]{figures/fig0-hook.png}}
...
\textbf{Figure 2.} Four-layer control architecture. Layers 1--3 embed in the system prompt as standing constraints. Layer 4 is invoked as an external tool at each conversational turn. Section 3 details each layer.
...
\textbf{Figure 4.} K-effect replication (P-stat). K-containing conditions: 5/5 covariance references; all non-K conditions: 0/5. Pattern held across Gemini and Claude.
...
\textbf{Table 13.} v1 factoring: profile × teaching route × consistency. Turn 2 (3 runs, A-mid): the LLM tutor guided the simulated learner profile to notice \(3+5=8\), \(3 \times 5=15\) within an already-computed expansion, constructing the factoring discovery from a confirmed skill.
...
\textbf{Figure 5.} v0 vs. v1 teaching routes (correlation coefficient). v0: all five runs converged on scatter plot; Turn 2 regressed to mean. v1: routes diverged by profile; obstacle detected and re-routed to confident ancestor.
...
\textbf{Figure 6.} Four-layer architecture mapped to KST-interpretable roles. The correspondence was identified post-hoc; it provides a theoretical vocabulary for design decisions made on experimental grounds.
