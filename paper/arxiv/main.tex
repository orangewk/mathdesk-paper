...
\textbf{Figure 1.} Four-layer control architecture. Layers 1--3 embed in the system prompt as standing constraints. Layer 4 is invoked as an external tool at each conversational turn. Section 3 details each layer.
...
\item
  \textbf{Knowledge state layer}: Represents the learner\textquotesingle s knowledge state K --- the acquired concept set --- as a node subset on the prerequisite graph. Only K affects tutoring paths; stumbling-block records and learning-style annotations produced no distinguishable effect.
\item
